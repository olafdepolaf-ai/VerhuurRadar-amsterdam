...
const FAQItem = ({ question, answer, link, linkText }: { question: string, answer: string, link?: string, linkText?: string }) => {
    ...
    return (
        <div className="border-b border-slate-100 last:border-0">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full py-3 px-4 text-left flex justify-between items-center hover:bg-slate-50 transition-colors focus:outline-none">
                <span className="font-semibold text-slate-700 text-sm pr-4 text-left">{question}</span>
                <span className={`text-red-600 flex-shrink-0 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg></span>
            </button>
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="p-4 pt-0 text-sm text-slate-600 leading-relaxed space-y-2 text-left">
                    ...
                    {link && <a href={link} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 text-red-600 hover:text-red-800 font-medium hover:underline text-left">{linkText || "Lees meer op amsterdam.nl →"}</a>}
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="w-full max-w-lg mt-8 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-fade-in-up delay-200 text-left">
            <FAQItem 
                ...
                answer={`Bij vakantieverhuur verhuurt de bewoner de hele woning aan toeristen. De bewoner is zelf niet thuis.\n\nDit is anders dan een Bed en Breakfast. Daarbij wordt maar een deel van het huis verhuurd en is de bewoner wel thuis.`}
            />
            <FAQItem 
