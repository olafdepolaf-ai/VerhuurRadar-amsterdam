      ...
      {!hasSearched ? (
        <div className="flex-1 overflow-y-auto"><div className="min-h-full flex flex-col items-center p-6"><div className="w-full max-w-2xl text-center flex-grow pt-10 mx-auto"><div className="mb-6 flex items-center justify-center gap-3"><VerhuurRadarIcon className="w-16 h-16" /><h1 className="text-6xl font-bold"><span className="text-slate-900">Verhuur</span><span className="text-red-600">Radar</span></h1></div><p className="text-lg text-slate-600 mb-8">Inzicht in alle vergunningen voor vakantieverhuur in Amsterdam.</p><AddressSearch onAddressSelect={handleAddressSelect} onUseLocation={handleUseLocation} /><div className="w-full max-w-2xl mx-auto space-y-2 mt-10">{recentPermits.length > 0 && <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest text-left"><span>Laatst verleend</span><a href="https://zoek.officielebekendmakingen.nl/" target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} title="Bekijk alle vergunningen op Officiële Bekendmakingen" className="text-slate-300 hover:text-slate-500 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg></a></div>}{recentPermits.map(p => <div key={p.id} onClick={() => handleRecentPermitClick(p.address)} className="bg-white border border-slate-200 rounded p-3 flex justify-between items-center cursor-pointer hover:bg-red-50"><span className="font-semibold text-sm">{p.address}</span><span className="text-xs text-slate-500">{formatRelativeDate(p.date)}</span></div>)}</div><TrendChart totalActiveCount={totalActiveCount} /><FAQSection /></div><footer className="w-full mt-16 pb-10 pt-8 border-t border-slate-200">
              <div className="flex flex-col items-center gap-1 text-sm text-slate-500">
                ...
                <span className="flex items-center gap-1.5 mt-2">Made with <AmsterdamHeartIcon className="w-3.5 h-3.5" /> in Amsterdam · Claude Code by <a href="https://www.linkedin.com/in/olaflemmers/" target="_blank" rel="noopener noreferrer" className="font-medium text-slate-700 hover:text-slate-900 transition-colors">Olaf Lemmers</a></span>
